...
const toNested = (data: ArrayLike<number | bigint | boolean>, dims: number[]): any => {
...
const i64 = (ort: any, arr: number[] | number[][], dims: number[]): any => {
...
export const feed = (ort: any, b: Batch): Record<string, any> => {
...
export const feedHead = (ort: any, hidden: number[][][] | any, b: Batch): Record<string, any> => {
...
const pickOutput = (out: Record<string, any>, names: string[]): any => {
...
const applyNumThreads = (ort: any, numThreads?: number): void => {
  try {
    const raw =
      numThreads ??
...
const isOomError = (e: unknown): boolean => {
...
export const loadNodeBundle = async (modelDirOrRepo: string, opts?: { subfolder?: string | null; localDir?: string; token?: string | null }): Promise<NodeBundle> => {
...
const baseUrlFor = (repoOrUrl: string, subfolder?: string | null): string => {
...
export const loadWebBundle = async (repoOrUrl: string, opts?: { subfolder?: string | null }): Promise<WebBundle> => {
...
export const createNodeProvider = async (modelDir: string, opts?: ProviderOptions): Promise<SessionProvider> => {
...
export const createWebProvider = async (modelUrl: string, opts?: ProviderOptions): Promise<SessionProvider> => {
