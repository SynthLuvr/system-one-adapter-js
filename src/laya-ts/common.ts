...
const pyJson = (v: unknown): string | undefined => {
...
const renderCriterion = (v: unknown): string => {
  return typeof v === "string" ? v : pyJson(v) ?? String(v);
}
export const renderOptions = (q: InternalQ): string[] => {
  if (q.t === "choice") {
...
export const buildSequence = (tok: TokenizerLike, state: unknown, q: InternalQ, maxLen = 512, headMaxLen = 192, optionOrder?: number[], truncateLeft = false): { ids: number[]; markers: number[] } => {
...
export const confidenceFromProbs = (p: number[]): number => {
  const k = p.length;
  if (k < 2) return 1.0;
...
export const collateItems = (batch: CollateItem[][], padId: number): CollatedBatch | null => {
