...
const isLatinCp = (cp: number): boolean => {
  return cp < 0x0250 || (0x1e00 <= cp && cp <= 0x1eff) || (0xff21 <= cp && cp <= 0xff3a) || (0xff41 <= cp && cp <= 0xff5a);
}

export const stateText = (state: unknown, maxChars = 4000): string => {
  const parts: string[] = [];
...
export const scriptProfile = (text: string): Record<string, number> => {
...
export const latinProfile = (text: string): LatinProfile => {
...
export const guessLatinLanguage = (text: string): string | null => {
...
export const analyse = (state: unknown): AnalyseResult => {
...
export const isEnglish = (state: unknown): boolean => {
