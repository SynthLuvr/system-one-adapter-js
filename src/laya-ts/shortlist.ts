...
const criteriaItems = (criteria: unknown): [string, unknown][] => {
...
const optionTexts = (items: [string, unknown][]): string[] => {
...
const queryText = (state: unknown, instructions?: unknown): string => {
...
const embeddings = async (embedFn: unknown, texts: string[]): Promise<number[][]> => {
...
const cosine = (query: number[], docs: number[][]): number[] => {
...
const rank = async (state: unknown, criteria: unknown, embedFn: EmbedFn, k: number, instructions?: unknown): Promise<{ labels: string[]; scores: number[] | null; passthrough: boolean; n: number }> => {
...
const subsetCriteria = (criteria: unknown, labels: string[]): unknown => {
...
export const shortlistChoice = async (state: unknown, criteria: unknown, embedFn: EmbedFn, k: number = DEFAULT_SHORTLIST_K, instructions?: unknown): Promise<string[]> => {
...
export const predictShortlist = async (agent: unknown, state: unknown, questions: Record<string, QuestionDef>, embedFn: EmbedFn, k: number = DEFAULT_SHORTLIST_K, predictKwargs: Record<string, unknown> = {}): Promise<SystemOneResult & { shortlist: Record<string, ShortlistMeta> }> => {
...
export const embedFnFromAgent = (agent: unknown, maxLength = 512, batchSize = 32): EmbedFn => {
