...
const byteUnicodeMaps = (): { b2u: Map<number, string>; u2b: Map<string, number> } => {
...
const maps = (): { b2u: Map<number, string>; u2b: Map<string, number> } => {
...
const bpeWord = (chars: string[], rank: Map<string, number>): string[] => {
...
export const bpeEncode = (vocab: Map<string, number>, merges: Map<string, number>, text: string): number[] => {
  const { b2u } = maps();
...
export const metaspaceEncode = (vocab: Map<string, number>, merges: Map<string, number>, text: string, unkId?: number, replaces: ReadonlyArray<readonly [string, string]> = [[" ", METASPACE_REPLACEMENT]]): number[] => {
...
export const encodeWithData = (data: TokenizerData, text: string): number[] => {
...
const hasNodeType = (node: unknown, want: string): boolean => {
...
const collectReplaces = (node: unknown, out: Array<[string, string]>): void => {
  if (!node || typeof node !== "object") return;
  const o = node as Record<string, unknown>;
  if (o["type"] === "Replace") {
...
export const parseTokenizerJson = (raw: unknown): TokenizerData | null => {
  try {
    const r = raw as {
...
export const loadTokenizerJson = async (pathOrUrl: string): Promise<TokenizerData | null> => {
  let raw: unknown;
