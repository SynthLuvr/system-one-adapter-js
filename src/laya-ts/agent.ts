...
const qidStr = (qid: string): string => {
  return JSON.stringify(qid);
}

export const checkQuestion = (qid: string, qdef: unknown): void => {
...
const sorted = (o: Record<string, number>): string[] =>
  Object.keys(o).sort();

export const toInternal = (qdef: QuestionDef): { t: "choice" | "score" | "noul"; ins: string; crit: unknown } => {
...
export const defaultTokenizer = (): TokenizerLike => {
  return {
    clsId: 101,
    sepId: 102,
    maskId: 103,
...
const tokenizerFromHF = (tokenizerJson: unknown): TokenizerLike | null => {
...
const tokenizerFromConfig = (cfg: AgentCfg): TokenizerLike | null => {
